...
cpc_var_select_eb.rf$importance  %>%
  as.data.frame() %>% # importance = IncNodePurity
  mutate ( variable = row.names(.)) %>%
  purrr::set_names( c("importance", "variable") ) %>%  
  dplyr::select ( all_of(c("variable", "importance")) ) %>%
  arrange ( -importance ) %>%
  knitr::kable()
```
...
cpr_predictor_var_select_count_per_researcher = Predictor$new(
  ...
  data = dplyr::select ( cpr_var_select_eb_researcher.rf , -geo, 
                         -count_per_researcher ),
  y = as.numeric(cpr_var_select_eb_researcher.rf$count_per_researcher))

cpr_imp_count_per_researcher <- FeatureImp$new(
  cpr_predictor_var_select,
  loss = "mae",
  n.repetitions = 10)
