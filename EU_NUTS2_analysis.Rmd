...
## Random Forest with Eurobarometer

```{r varselecteb, eval=TRUE, message=FALSE, echo=FALSE}
#count per capita
cpc_var_select_eb_df <- eurostat_eurobarometer_complete_data  %>% 
  dplyr::select ( -researchers_total, 
                  -population_total,
                  -count_per_million,
                  -count_per_thousand_researchers,
...
## Tuning this model
set.seed(2019) # for reproducability, the random number seed 2019 is used.
cpc_best_mtry_var_eb_select <- tuneRF(
...
cpc_var_select_eb.rf <- randomForest(count_per_capita ~ . ,
                            dplyr::select ( cpc_var_select_eb_df,
                                            -all_of("geo")),
                            mtry = as.numeric(
                              cpc_best_mtry_var_eb_select[1]),
                            ntree=5000 )
```

``` {r randomforestebresults, echo=FALSE}
