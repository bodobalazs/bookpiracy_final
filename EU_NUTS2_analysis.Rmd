---
title: "European Union Regional Models"
...
export_summs(percapita_qplm3, percapita_qplm4, percapita_qplm5, percapita_qplm6,percapita_qplm7,
             ...
             model.names = c('Model 7','Model 8', 'Model 9', 'Model `10', 'Model 1'),
             to.file = "docx", 
             file.name = "trashcan/table7.docx")


cor(percapita_qplm3$fitted.values, eurostat_complete_data$count_per_million)^2
...
export_summs(perresearcher_qplm1, perresearcher_qplm2,perresearcher_qplm3, 
             digits=3, 
             statistics = c("null.deviance", "deviance"),
             model.names = c('Model 12','Model 13', 'Model 14'),
             to.file = "docx", 
             file.name = "trashcan/table8.docx")
             )
...
#count without R&D
count_plm2 <- glm (count ~ log(gdp_pps) +
                     researcher_employment_pct +
                 disposable_income +
                 edu_attainment_total +
                 internet_use_banking_pc, 
...
#with R&D
count_plm3 <- glm (count ~ log(gdp_pps) +
                 researcher_employment_pct +
                 disposable_income +
                 edu_attainment_total +
                 internet_use_banking_pc +
...
pdf("paper/PLOS/S3_Table.pdf", height=11, width=24)
grid.table(export_summs(count_qplm2,count_qplm3,count_qplm4, digits=10, statistics = "all"))
dev.off()
...
export_summs(interaction_qplm1, 
             digits=3, 
             statistics = c("null.deviance", "deviance"),
             model.names = c('Model 15'),
             to.file = "docx", 
             file.name = "trashcan/table9.docx")
             )
...
```{r export interaction graph}
#export graph
setEPS(width = 12, pointsize = 20)
postscript("paper/fig5.eps")
plot_model(interaction_qplm1, 
           type = "eff", terms=c("researcher_employment_pct[0.1,1.9]",
                                 "gdp_pps [10000,150000]"))
dev.off()
...
#source ( 'helper_code/all_liniear_regression_model_combinations.R')
load(file.path("data", "lin_model_summaries.rda"))
significant_model_summaries %>%
  dplyr::select ( y, x1, x2, adj.r.squared, statistic, AIC ) %>%
  knitr::kable( )
# not all summary statistics are printed
```

```{r linregsummary, out.width='90%', warnings=FALSE, message=FALSE, echo=FALSE}
# To re-run the many models with the same dataset as used here.
#uncomment the next line if you want to rerun the lingreg models
#source ( 'helper_code/all_liniear_regression_model_combinations.R')
plot(review_coefficients_plot)
```

We created the models on scaled variables for comparability. We selected all models where the single or both coefficients were significant (at t = 1.96 level) and plotted the size of coefficient. The more significant coefficients are darker.  We removed some illogical models, like the ones that contained two versions of the GDP variable.

Modelling `count` results in more selected models, because the most trivial variables (such as population size or number of employed people) are always selected.  Briging to the left hand side the size of the region or the size of the researcher population results in less, but more usable models. 

The `area_land_filled` variable is negatively correlated to the explained variables.  The reason is that the NUTS regions are homogenized mainly for population size, which results in very large land areas in scarcely populated, rural areas that usually have very few researcher jobs. 

This approach shows results which are consistent with the deductive models.  Income, the percentage of knowledge workers in the workforce has positive effects, online proficiency (in all forms) has a negative effect. What is new are the emergence of tow of the new `EUROBAROMETER` variables: the share of population who visited a public library at least once in the last 12 months (with a negative sign), and the percentage of students in he population (with a positive one).
...
As we have seen in the deductive models, the linear regressionn formulation is not necessarily the best one, we made this comparison because we found it relatively easy to read.  This variable screening or selection method can be made in a more robust way with random forest models. The model summaries of the random forest models are more dificult to interpret and to compare with our deductive models.

## Random forest models

In the following we created three sets of analysis: one that uses `count_per_researcher` as the dependent variable, one that uses `download_per_capita` as dependent variable, and one that uses raw `count` as a dependent variable.
...
cpc_predictor_var_select = Predictor$new(
  cpc_var_select.rf,
  data = dplyr::select ( cpc_var_select_df , 
                         -geo, -count_per_capita),
  y = as.numeric(cpc_var_select_df$count_per_capita))

cpc_imp <- FeatureImp$new(cpc_predictor_var_select, 
                          loss = "mae", 
                          n.repetitions = 10)
plot(cpc_imp)
```
...
```{r feature_importance_eurobarometer export Graph, out.width='100%', eval=FALSE}
#export graph
postscript("paper/fig6.eps")
plot(cpc_imp_eb_count_per_capita)
dev.off()
```
...
## Comparison of the EUROBAROMETER models
...
export_summs(percapita_qplm3_1, 
             ...
             to.file = "docx", 
             file.name = "trashcan/table10.docx")
