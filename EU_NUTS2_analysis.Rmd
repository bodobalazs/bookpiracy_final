...
percapita_qplm5 <- glm (count_per_million ~ log(gdp_pps) +  researcher_employment_pct +
                 internet_purchases_last_year_pc +
                 log(disposable_income) +
                 edu_attainment_total, data = complete_df, quasipoisson)
...
jtools::export_summs(percapita_qplm3, perresearcher_qplm1, 
                     count_qplm2, model.names=c('percapita', 'perresearcher','count'),digits=10, statistics = "all")
```
...
```{r raw_caount models, echo=FALSE, eval=TRUE}
...
raw_imp <- FeatureImp$new(raw_predictor_var_select, loss = "mae", n.repetitions = 100)
```
...
perresearcher_lm <- lm (count_per_thousand_researchers ~ gdp_pps  + disposable_income+ edu_attainment_total+ internet_use_banking_pc                , data = complete_df)
