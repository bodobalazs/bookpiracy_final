...
author: "Balazs Bodó, Daniel Antal"
date: "04/05/2020"
output:
...
library(colorRamps) # for visualization
...
load("data/all_data_for_analysis.rda")
```

# Introduction

The following document contains the analysis of illegal library downloads on an European NUTS2 regional level. We used EUROSTAT and EUROBAROMETER data sources to compile a dataset with 265 NUTS2 regions, 7 versions of the explained variable, 17 explanatory variables, and one metadata variable (the regional code.) We describe the  dataset used in the  analysis in a separate document in this repository.
...
summary(complete_df$count)
histogram(log10(complete_df$count), nint=100, type="count")
boxplot(complete_df$count)
```

Some of the outlier regions are as expected, big, metropolitan regions, such as inner London, with large population s, and a strong concentration of knowledge-intensive activities. There are, however, exceptions to this rule, where regions without significant urban centers, or educational, research capacities demonstrate unusually high download volumes. We identified a number of possible reasons for these anomalies:

* there might be issues with the translation of IP addresses to Geoocation. We used the Maxmind service to assign coordinates to IP addresses, and that service may vary in terms of resolution within different European countries, and the ability to correctly assign location fir different internet service providers. In this latter case it is possible that lacking better information, whole IP ranges resolve to, for example, the HQ address of the provider, rather than the approximate location of the user. This is a well-known issue in general, and a potential source of noise in our case as well.

* We did our best to identify Virtual Private Networks, TOR exit nodes, and other traffic sources which may mask the true location of the downloader. Such information may not always be available, therefore it is possible that we failed to identify traffic sources as VPNs and thus the volume of traffic associated with a particular IP address belongs to an unknown number of users from other locations.

* Last, but not least, though we tried our best to identify bots and other automated downloads in the dataset (by, for example, filtering repeated downloads from the same Ip of the same book within a given time-window), we did not identified the automatic scraping of parts of the shadow library. In other parts of the dataset we have evidence for such automated, scraper generated traffic, but on smaller scale this might also produce unexpected outliers in the European dataset.
...
The regionalized Eurobarometer variables were created [@antal_2020_eb_bookpiracy] from the microdata of the Eurobarometer 79.2 survey.  Because the Eurobarometer survey samples are designed to be representative of EU countries and some important regions (i.e. Great Britain and Northern Ireland, East- and West-Germany, the Republic of Cyprus and the Turkish Cypriot Community separately, too), some NUTS2 regions are undersampled, others are oversampled, which leads to inevitable, but not systematic bias.

We used a ‘dirty’ re-weighting to make regionalized sums consistent by re-centering the original, national post-stratification weight around the value of 1 in each region.  (Their mean value should be 1 in the national sample, but because the NUTS sub-samples are not necessarily representative for the region, the mean weight may be lower or higher than 1.)

These variables proved to be important additions to the analysis, because they filled important data gaps about reading habits (positively correlated with pirate book downloads), public library visiting habits (negatively correlated) and perceived library conditions (negatively correlated with perceived poor conditions).  We also used a science and technology attitude question from the same survey about opinions on giving open access to publicly funded scientific research results.  While this may be related to the narrative along book piracy, differences in opinion were not significant, most likely because this topic may be have strong opinions among the researcher community, but not easy to understand for the general population of Europe. 

## Spatial auto-correlation of the download counts
...
download_data <- readRDS( file.path("raw_data",
                                    "downloads_nuts_2016.rds") ) 
...
set.seed(2019)
moran_count_results <- spdep::moran.mc(x = moran_i_spdf %>%
                  dplyr::select ( count ) %>% 
                  unlist () %>%
                  as.numeric(),       
                ww, 
...
set.seed(2019)
moran_gdp_results <- spdep::moran.mc(x = moran_analysis_spdf_gdp_pps %>%
...
The coefficients are relatively consistent, with the exception of the level of daily internet users in the area.  The most important variables are the level of GDP, number of researchers in the area and HR resources in science and technology in the area (all increases the number of pirate book downloads), and disposable income, which decreases it.

## Random forest models

In the following I created three sets of analysis: one that uses count per researcher as the dependent variable, One that uses download per capita as download variable, and one that uses raw count as a dependent variable.
...
#count per capita
cpc_var_select_df <- complete_scaled %>% 
...
cpc_var_select.rf$importance  %>% as.data.frame() %>%
...
# takes a long time to run this code, we ran it and saved the results 
# in images_graphs/000001.png

```

Our internet banking use variable is a higher quality variable than the regionalized Eurobarometer variables about visiting public libraries in the previous 12 months, or reading a book, but they are roughly similarly important in predicting per capita download counts. 
...
run_in_function <- function() {
  interact <- Interaction$new(cpc_predictor_var_select)
  plot(interact) #causes knitr issues, something is wrong with the plot,  saved it and print it from file.

}
# Re-run:  run_in_function()
# Saved results here:
knitr::include_graphics('images_graphs/percapita_interactions_0424.png')
```

In interaction, book reading habits, and the perceived negative quality of the local public library supply are rather important variables. 
...
cpc_best.rf$importance  %>% as.data.frame() %>%
  arrange ( -IncNodePurity )
...
cpc_correlations_bestmodel <- complete_df %>%
  dplyr::select (count_per_capita, 
                 gdp_pps_hab, 
                 researcher_employment_pct,
                 disposable_income,
...
To be able to use poisson and quasipoisson models I use a count per million variable, which is the rounded value of count per million inhabitants.

I test models with and without the scitechemployment percentage, and found that that variable comes with horrible multicollinearity. so i decided to drop it. I also switched  gdp_pps_hab to GDP_pps because though it had the most importance, it  also suffered from multicollinearity.
...
percapita_qplm4 <- glm (count_per_million ~ log(gdp_pps) +  researcher_employment_pct +
                 internet_use_banking_pc
                , data = complete_df, quasipoisson)
...
export_summs(percapita_qplm4, percapita_qplm3, percapita_qplm5, digits=3, statistics = c("null.deviance", "deviance"))
summary (percapita_qplm4, percapita_qplm3)
vif()
```

The  model offers the following  findings:

- gdp is has a significant positive effect in the poisson regression, but has not in the qpoisson model. 

- the per capita dowloads grow with the a higher percentage of researchers in the labour pool, and with higher disposable income. These point to a structural demand effect (economic/ research activity drives demand for scientific literature), and and individual demand effect:  higher disposable income does not lower piracy, but actually creates more demand.

- on the other hand, per capita downloads are moderated by better online skills. The negative effect of online banking use may point to a higher use of legal sources, such as online and offline purchases, but we should also consider that online proficiency provides the skills to hide the online traces of illegal activities via the use of VPNs and Tor brosing.
...
correlation::correlation(complete_df)
```
...
cpr_var_select_df <- complete_scaled %>% 
  dplyr::select ( -researchers_total, 
                  -population_total,
                  -count_per_million,
                  -count_per_capita,
...
cpr_interact_functin <- function() {
 ...
 save ( cpr_interact, file = file.path('raw_data', 
                                  'perresearcher_interactions_0424.rda'))
}

#Saved the result here:
knitr::include_graphics(file.path('images_graphs', 
                                  'perresearcher_interactions_0424.png')
                        )
```
...
cpr_correlations_bestmodel <- complete_df %>%
  dplyr::select (count_per_researcher, 
                 internet_use_banking_pc,
                 disposable_income,
                 edu_attainment_total,
...
perresearcher_plm2 <- glm (count_per_thousand_researchers ~ 
                             gdp_pps_hab  + disposable_income +
                             edu_attainment_total+gerd+
                             internet_use_banking_pc+hr_scitech_pct+
                             is_read_book+is_visit_public_library, 
                           data = complete_df, poisson )
...
- the R&D expenditure, internet proficiency and the higher education attainment.
...
count_plm2 <- glm (count ~ gdp_pps +  researcher_employment_pct +
                 ...
                 edu_attainment_total +
                 internet_use_banking_pc +
                 is_visit_public_library + is_read_book, 
                 data = complete_df, poisson )
...
export_summs(count_qplm2,
             count_qplm3,
             count_qplm4, digits=10, statistics = "all")
...
if we compare the three models (per capita, per researcher, and absolute download counts, all modeled as quasipoission), we find that consistent results.
researcher employment has a positive effect, internet savvyness has a negative effect. 

in one model more GDP results in more downloads, but we also see that higher R&D expenditure can lower download counts. 

```{r 3 best models, echo=FALSE}
...
To check the robustness of these models, we  also did simple linear regression for all 3 dependent variables. They do not yield different results from the qpoission models, but their error terms are much uglier.
...
To further check robustness, we conducted the same exercise for raw (not scaled) variables. The results are below, they do not yield different results.

```{r raw_count models, echo=FALSE, eval=TRUE}

#lets do a random forest for count variables first

raw_var_select_df <- complete_df %>% 
  dplyr::select ( -researchers_total, 
                  -population_total,
                  -count_per_million,
                  -count_per_capita,
...
raw_var_select.rf$importance  %>% as.data.frame() %>%
  arrange (-IncNodePurity)

plot(raw_imp)
```

The random forest on the raw count variable identified the same independent variables as relevant. One exception was the employment total variable, but that was highly correlated with the gdp_pps variable, so we had to omit it from the the liner regression.

Here are the results of the per capita, per researchers, and raw count simple linear models.

``` {r raw linreg models}

percapita_lm <- lm (count_per_million ~ gdp_pps +  researcher_employment_pct +
                 disposable_income +
                 edu_attainment_total +
                 internet_use_banking_pc,
                 data = complete_df)

perresearcher_lm <- lm (count_per_thousand_researchers ~ gdp_pps  +
...
vif(count_lm)
#summary(count_lm4)
export_summs(percapita_lm,perresearcher_lm,count_lm, digits=10, statistics = "all",model.names=c('percapita', 'perresearcher','count'))
plot(count_lm)
...
Finally, we check the interaction of income level (GDP_PPS) and researcher employment with a simple interaction model. 
